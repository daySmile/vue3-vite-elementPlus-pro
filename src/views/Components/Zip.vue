...
export default {
  ...
  setup() {
    ...
    const handleExportZip = () => {
      import("../../utils/exportFile/Export2Zip").then(zip => {
        const tHeader = ['日期', '姓名', '省份', '市区', '地址', '邮编']
        const filterVal = ['date', 'name', 'province', 'city', 'address', 'zip']
        const data = formatJson(filterVal, state.tableData);
        let arrList = [{tHeader, data}]
        // 导出zip包含多个xlsx文件
        zip.export_xlsx_to_zip(arrList, "样例数据包" + execelDate);
      });
    }
